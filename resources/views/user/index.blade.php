                            ...
                            <tbody>
                                @foreach($data as $no => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <select name="status" style="height: 30px; font-size: 12px;"  onchange="change_status(this, '{{ route('user.inactive', $item->id) }}', '{{ route('user.active', $item->id) }}')" class="form-control">
...
<script type="text/javascript">
    ...
    function change_status(ini, url_inactive, url_active)
    {
        var sel = $(ini);

        if(sel.val() ==1 ){
            bootbox.confirm({
                message: "Apakah anda ingin mengaktifkan User ini ?",
                buttons: {
                    confirm: {
                        label: 'Yes',
